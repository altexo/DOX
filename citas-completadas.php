...
$hola = "Select * from datos where idpaciente = ".$rw['id']." and status ='autorizar-factura' and autorizar = '1' order by inicio desc";
$idpaciente=$rw['id'];
$sql = mysql_query($hola);

$resultado = mysql_query("Select * from datos where idpaciente = ".$rw['id']." and autorizar='0' and status ='cita-hecha' and inicio >= '".$hoy."' order by inicio desc");
...
<a href="citas-facturas.php">Mis Facturas</a>
       ...
       <table data-toggle="table" data-cache="false" data-height="299" class="table table-striped">
        <thead> <tr>
           ...
           <td align="center" scope="col"><strong>Clave de reserva</strong></td>
		...
		?>
		 ...
		 $hola2 = "Select * from doctor where id = ".$row['iddoctor']."";
		  ...
		  echo $t." ".$row2['nombre']." ".$row2['a_paterno']." ".$row2['a_materno'];
		    ?></td>
           <td><a href="facturar-ahora.php?id=<? echo $row['id']; ?>&idpaciente=<? echo $idpaciente; ?>&fecha=<? echo $f; ?>&hora=<? echo $h; ?>&doctor=<? echo $t." ".$row2['nombre']." ".$row2['a_paterno']." ".$row2['a_materno']?>"></a><? echo $row['clavereserva'] ?></td>
           <td><a href="facturar-ahora.php?id=<? echo $row['id']; ?>&amp;idpaciente=<? echo $idpaciente; ?>&amp;fecha=<? echo $f; ?>&amp;hora=<? echo $h; ?>&amp;doctor=<? echo $t." ".$row2['nombre']." ".$row2['a_paterno']." ".$row2['a_materno']?>">Facturar Ahora</a></td>
         </tr>
		...
		{ ?> <tr> <td colspan="5" > <?
			echo "<p>Por el momento no hay ninguna cita registrada como 'completada'.</p>";
